# -*- coding: utf-8 -*-
"""Original Manuscript Code.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1uaTz2HdaB1Di1Sm7OUi25vUeHFgqbzxu
"""

# =========================
# Neutrosophic MCDM — from scratch (aligned + non-negative IWNS)
# =========================
...
# ---------- Global styling ----------
# Global typography: prefer Cambria, fall back gracefully
plt.rcParams.update({
    "font.family": "serif",
...
def set_color_style():
    ...
    mpl.rcParams.update({
        ...
        "font.family": "serif", "font.size": 14, # Set font size here as well
...
# Function to create directories and save figures
...
# -----------------------------
# Problem data (SVNS baseline)
# -----------------------------
alternatives = ["Treatment A", "Treatment B", "Treatment C"]
criteria = ["Effectiveness", "Side Effects (Tolerability)", "Affordability", "Recovery Time", "Patient Satisfaction"]
ALT_SHORT = ["A", "B", "C"]; ALT_MAP = dict(zip(ALT_SHORT, alternatives))
ALT_ORDER = alternatives # Define ALT_ORDER here
...
# Axis labels: Cambria, not bold, black
ax.set_xlabel("", fontfamily="Cambria", fontweight="normal", color="#000000")
ax.set_ylabel("", fontfamily="Cambria", fontweight="normal", color="#000000")

# Tick labels (criteria + alternatives): Cambria, bold, black
for t in ax.get_xticklabels() + ax.get_yticklabels():
    t.set_fontfamily("Cambria")
    t.set_fontweight("bold")
    t.set_color("#000000")

# (Optional) Title in the same style
ax.set_title("", fontfamily="Cambria", fontweight="bold", color="#000000")

plt.tight_layout()
save_figure(fig, "T_heatmap_quick") # Save the figure
plt.show()
...
# -----------------------------
# Heatmap of T (pre-fusion) — Cambria + bigger in-cell text + save
# -----------------------------
import matplotlib as mpl

# Extract T values safely (fix: use applymap, not map)
df_T = df_ndm.applymap(lambda x: float(x[0]))
...
hm = sns.heatmap(
    ...
    fmt=".2f",
    cmap="viridis",
    linewidths=0.8,          # slightly thicker grid lines
    linecolor="white",
    square=True,
    cbar_kws={"shrink": 0.8},
    ax=ax,
    annot_kws={
        "fontsize": 14,      # bigger numbers inside boxes
        "fontweight": "bold",
        "color": "black",
        "fontname": "Cambria"
    }
...
# Axis labels & title (Cambria, not bold y-axis)
ax.set_title("T-values Heatmap (NDM)", fontsize=16, fontweight="bold", fontname="Cambria", pad=12)
ax.set_xlabel("", fontsize=14, fontname="Cambria", fontweight="normal")
ax.set_ylabel("", fontsize=14, fontname="Cambria", fontweight="normal") # Not bold

# Tick labels (Cambria + readable size)
ax.set_xticklabels(ax.get_xticklabels(), rotation=20, ha="right", fontsize=13, fontname="Cambria")
ax.set_yticklabels(ax.get_yticklabels(), rotation=0, fontsize=13, fontname="Cambria")

# Colorbar ticks (Cambria)
cbar = ax.collections[0].colorbar
cbar.ax.tick_params(labelsize=13)
for lbl in cbar.ax.get_yticklabels():
    lbl.set_fontname("Cambria")
...
# Save in current folder in multiple formats
...
def dst_fuse_triplets(triplets_list):
    ...
    return (bp["G"], bp["Θ"], bp["B"]), bp  # (T,I,F), bpa
...
# -----------------------------
# Normalization (benefit-style for all)
# T: ascending; I,F: descending mapped to benefits
# -----------------------------
...
# -----------------------------
# Non-negative IWNS (benefit-sum; in [0,1])
# -----------------------------
def iwns_scores_benefit(ivns_norm, weights, alpha=1.0, beta=1.0, gamma=1.0):
    ...
    for i in range(n_alt):
        ...
        out[i] = s  # bounded in [0,1]
    return out

# -----------------------------
# Extended TOPSIS on benefits (all three comps are benefits after norm)
# Ideal = max for all; Anti-ideal = min for all
# -----------------------------
def topsis_rc_from_ivns_benefit(ivns_norm, weights):
    ...
    RC = Dm / (Dm + Dp + 1e-12)  # in [0,1]
    return RC

# -----------------------------
# Pipeline: Experts → DST → IVNS → Normalize → IWNS + TOPSIS
# -----------------------------
def run_pipeline_expertfusion_to_rank(experts_triplets, delta=0.05, weights=None,
    ...
    # 3) Normalize (all benefits)
    ivns_norm = normalize_ivns_matrix(ivns)

    # 4) Non-negative IWNS (benefit sum)
...
plt.title("Sensitivity: TOPSIS Relative Closeness by Alternative")
plt.ylabel("RC (0–1)", fontweight='normal', color='black') # Not bold
plt.tight_layout();
save_figure(plt.gcf(), "Sensitivity_TOPSIS_boxplot") # Save the figure
plt.show()

# -----------------------------
# Monte Carlo — full pipeline
# -----------------------------
...
for alt in alternatives:
    ...
    # Axis labels in very dark black and bold, same size as other plots
plt.xlabel("Relative Closeness (RC)", color="#000000", fontweight="bold", fontfamily="Cambria", fontsize=14)
plt.ylabel("Cumulative Probability", color="#000000", fontweight="bold", fontfamily="Cambria", fontsize=14)
...
plt.tight_layout();
save_figure(plt.gcf(), "MonteCarlo_RC_CDF") # Save the figure
plt.show()


plt.figure(figsize=(8,5))
sns.boxplot(data=df_mc)
plt.ylabel("Rank (lower is better)", fontweight='normal', color='black') # Not bold
plt.title("Monte Carlo: Rank Distribution (full pipeline)")
plt.tight_layout();
save_figure(plt.gcf(), "MonteCarlo_Rank_Distribution") # Save the figure
plt.show()

# ============================================================
# Baselines & Comparative Evaluation
# labels: [14], [15], [16], proposed
# ============================================================

M_CRISP = "Crisp TOPSIS"; M_FUZZY = "Fuzzy TOPSIS"; M_IFS = "IFS–TOPSIS"; M_NEUTRO = "Neutrosophic TOPSIS (no DST)"; M_PROP = "Proposed Method"
#M_CRISP = "[53]"; M_FUZZY = "[54]"; M_IFS = "[55]"; M_PROP = "proposed"
...
def ifs_topsis(ndm_mat, weights):
    ...
    NU_plus, NU_minus = NU_n.min(axis=0), NU_n.max(axis=0)  # cost style
    PI_plus, PI_minus = PI_n.min(axis=0), PI_n.max(axis=0)  # cost style
...
def neutrosophic_topsis_no_dst(ndm_mat, weights, delta=0.05):
    """
    Neutrosophic TOPSIS without DST fusion.
    Uses IVFS transformation + normalization + TOPSIS but skips DST.
    This isolates the contribution of DST in the proposed method.
    Based on: Ye, J. (2014). A multicriteria decision-making method using
    aggregation operators for simplified neutrosophic sets. Journal of
    Intelligent & Fuzzy Systems, 26(5), 2459-2466.
    """
    m, n = len(alternatives), len(criteria)
    # Use triplets as-is (no DST), inflate to IVNS
    ivns = np.empty((m, n), dtype=object)
    for i in range(m):
        for j in range(n):
            ivns[i,j] = inflate_to_ivns(ndm_mat[i,j], delta)
...
# ---------- Q1-grade figures (color-blind safe + print-friendly + vector export) ----------
import matplotlib as mpl
import matplotlib.pyplot as plt
import numpy as np
import seaborn as sns

# Use your short method labels
# M_CRISP = "Crisp TOPSIS"; M_FUZZY = "Fuzzy TOPSIS"; M_IFS = "IFS–TOPSIS"; M_NEUTRO = "Neutrosophic TOPSIS (no DST)"; M_PROP = "Proposed Method"
METHOD_ORDER = [M_CRISP, M_FUZZY, M_IFS, M_NEUTRO, M_PROP]


# Color-blind safe palette (Okabe–Ito) + distinct hatch patterns for grayscale printing
PALETTE = {
    M_CRISP: "#0072B2",   # blue
    M_FUZZY: "#E69F00",   # orange
    M_IFS:   "#009E73",   # green
    M_NEUTRO: "#CC79A7",  # pink/mauve
    M_PROP:  "#D55E00",   # vermilion
}
HATCHES = {
    M_CRISP: "",
    M_FUZZY: "//",
...
def _pub_style():
    ...
    mpl.rcParams.update({
        ...
        "font.family": "serif",
        "font.size": 14, # Set font size here
        "axes.labelsize": 14, # Set label size here
        "axes.titlesize": 15, # Set title size here
        "legend.fontsize": 12, # Set legend size here
        "xtick.labelsize": 13, # Set tick label size here
        "ytick.labelsize": 13, # Set tick label size here
    })

# Moved save_figure function outside of _pub_style to be accessible
# def _save(fig, name: str):
#     for ext in ("pdf", "svg", "png"):
#         fig.savefig(f"{name}.{ext}", bbox_inches="tight")

def _annotate_bars(ax, bars):
    """Put 2-dec labels above bars; add a star on the tallest bar per x-group."""
    # Find group maxima by x-center
    by_x = {}
    for b in bars:
    ...
    for x, group in by_x.items():
        ...
        for g in group:
            ...
            ax.annotate(f"{h:.2f}",
                        ...
                        ha="center", va="bottom", fontsize=12, color="black") # Adjusted fontsize here
        # star on best
        ...
        ax.annotate("",
                    ...
                    ha="center", va="bottom", fontsize=14, color="black") # Adjusted fontsize here

def plot_rc_bars(comp_df, scenario_label, fname, highlight=M_PROP):
    """
    Grouped bar chart of RC by method × alternative.
    - Color + hatch per method
    - Star marks best method per alternative
    - Proposed method outlined for emphasis
    """
    _pub_style()
    ...
    for k, method in enumerate(METHOD_ORDER):
        ...
        # Optional: add slim black markers (useful in grayscale)
        ax.plot(x + offs, vals, "o", ms=2.5, color="black", zorder=4)

    _annotate_bars(ax, all_bars)

    ax.set_title(scenario_label) # Use the scenario_label directly as the title
   # ax.set_xlabel("Alternative")
    ax.set_ylabel("RC (0–1)", fontweight='bold', color='black', fontsize=14) # Make Y-axis label bold, black, and bigger

    ax.set_ylim(0, 1.0)
    ax.set_xticks(x)
    #ax.set_xticklabels(ALT_ORDER)
    # X tick labels: Cambria, bold, pure black
    ax.set_xticklabels(ALT_ORDER, fontsize=14, fontweight='bold', color='black') # Make X-axis tick labels bold, black, and bigger
    ...
    # Tidy spines, keep bottom/left
    for s in ("top", "right"):
        ax.spines[s].set_visible(False)

    # Compact, multi-column legend above plot
    leg = ax.legend(
        title="",
        ncols=5,
        loc="upper center",
    ...
    for text in leg.get_texts():
        text.set_color("black")
    # Change legend title color to black
    leg.get_title().set_color("black")


    plt.tight_layout()
    save_figure(fig, fname) # Save the figure
    plt.show()

def plot_rank_heatmap(ranks_df, scenario_label, fname):
    """
    Heatmap of ranks (lower is better).
    Uses reversed palette so rank=1 is darkest (high emphasis).
    """
    _pub_style()
    R = ranks_df.loc[ALT_ORDER, METHOD_ORDER]

    fig, ax = plt.subplots(figsize=(7.8, 3.6))
    # vmin=1; vmax dynamic to number of methods
    vmax = int(np.nanmax(R.to_numpy()))
    cmap = sns.color_palette("light:navy", as_cmap=True)  # aesthetically calm; invert logic with norm
    im = sns.heatmap(
        R, ax=ax,
        annot=True, fmt=".0f",
        cmap="YlGnBu_r",   # reversed so 1 is dark
        vmin=1, vmax=vmax,
        cbar_kws={"shrink": 0.8, "label": "Rank (1 = best)"},
        linewidths=0.5, linecolor="#FFFFFF"
    )
    ax.set_title(f"Ranks by Method — {scenario_label}")
    ax.set_xlabel("Method")
    ax.set_ylabel("Alternative", fontweight='normal', color='black') # Not bold
...
def plot_rc_panels(compA, compB, fname, highlight=M_PROP):
    """
    Optional: create a two-panel figure (Scenario A & B) to save space.
    """
    _pub_style()
    fig, axes = plt.subplots(1, 2, figsize=(12.5, 4.6), sharey=True)
    for ax, comp_df, title in zip(
        axes,
        [compA.loc[ALT_ORDER, METHOD_ORDER], compB.loc[ALT_ORDER, METHOD_ORDER]],
        ["A. Single NDM (no fusion)", "B. Multi-expert (mean vs DST)"]
    ):
        A, M = comp_df.shape
        x = np.arange(A)
        width = 0.16
    ...
    axes[0].set_ylabel("RC (0–1)", fontweight='normal', color='black') # Not bold

    # One shared legend for both panels
    handles, labels = axes[0].get_legend_handles_labels()
    fig.legend(
        handles, labels, title="Method",
        ncols=5, loc="upper center", bbox_to_anchor=(0.5, 1.14),
...
# Optional consolidated two-panel RC figure
...
# =========================================================
# Reporting tables (exact prints requested)
# =========================================================

# 1) IVFS from baseline NDM (pre-DST)
delta_report = 0.05
...
# 2) DST-aggregated intervals (Truth bounds only) from the single-run pipeline
ivns_after_dst = res["ivns"]
dst_T_only = np.empty((len(alternatives), len(criteria)), dtype=object)
for i in range(len(alternatives)):
    for j in range(len(criteria)):
        dst_T_only[i, j] = ivns_after_dst[i, j][0]  # (T_low, T_high)
df_dst = pd.DataFrame(dst_T_only.tolist(), columns=criteria, index=alternatives)

# 3) Normalized intervals (Truth bounds only)
...
# ============================
# 3D Bar Chart — Rank Frequencies
# ============================
from mpl_toolkits.mplot3d import Axes3D  # noqa
import numpy as np

# Build frequency table: counts[alt_idx, rank-1]
rank_levels = [1,2,3]
...
ax.set_title("Monte Carlo Rank Frequencies (N = 1000)", fontsize=16, fontweight="bold")
ax.set_xlabel("Alternative", fontsize=13)
ax.set_ylabel("Rank", fontsize=13)
ax.set_zlabel("Frequency", fontsize=13)
...
leg = ax.legend(handles=handles, title="Ranks", loc="center left", bbox_to_anchor=(1.02, 0.5), frameon=True)
